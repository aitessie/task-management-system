...
/**
 * Сервис для поиска по названию.
 */
@Service
public class TitleSearcher extends BaseSearcher {

    public TitleSearcher(TaskRepository taskRepository) {
        super(taskRepository);
    }

    @Override
    public Page<TaskEntity> search(String searchParam, Pageable page) {
        return taskRepository.findByTitleContains(searchParam, page);
    }

    @Override
    public TaskFilterColumnName getColumnName() {
        return TaskFilterColumnName.TITLE;
    }
}
